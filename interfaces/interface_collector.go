...
var (
    ...
    powerInlineDesc        *prometheus.Desc
    cefStatusDesc          *prometheus.Desc
)
...
func init() {
    ...
    powerInlineDesc = prometheus.NewDesc(prefix+"power_inline_watts", "Power over Ethernet in watts", l, nil)
    cefStatusDesc = prometheus.NewDesc(prefix+"cef_status", "CEF status (1 if up, 0 if down)", l, nil)
}
...
func (c *interfaceCollector) Collect(client *rpc.Client, ch chan<- prometheus.Metric, labelValues []string) error {
    ...
    powerOut, err := client.RunCommand("show power inline")
    if err != nil {
        return err
    }
    powerMap, err := ParsePowerInline(client.OSType, powerOut)
    if err != nil {
        if client.Debug {
            log.Printf("Parse power inline for %s: %s\n", labelValues[0], err.Error())
        }
    }

    cefOut, err := client.RunCommand("show cef interface")
    if err != nil {
        return err
    }
    cefMap, err := ParseCefInterface(client.OSType, cefOut)
    if err != nil {
        if client.Debug {
            log.Printf("Parse cef interface for %s: %s\n", labelValues[0], err.Error())
        }
    }

    for i, item := range items {
        if power, ok := powerMap[item.Name]; ok {
            items[i].PowerInlineWatts = power
        }
        if cefStatus, ok := cefMap[item.Name]; ok {
            items[i].CefStatus = cefStatus
        }
    }
    ...
    for _, item := range items {
        ...
        ch <- prometheus.MustNewConstMetric(powerInlineDesc, prometheus.GaugeValue, item.PowerInlineWatts, l...)
        ch <- prometheus.MustNewConstMetric(cefStatusDesc, prometheus.GaugeValue, float64(cefStatus), l...)
    }
    return nil
}